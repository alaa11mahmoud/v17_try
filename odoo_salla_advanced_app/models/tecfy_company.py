...
class TecfyCompany(models.Model):
    ...
    tecfy_product_update_to_salla = fields.Boolean(required=False, string="Update Product To Salla", help="if checked, then the connector will update product to salla when create product in Odoo")
    tecfy_product_add_to_salla = fields.Boolean(required=False, string="Add Product To Salla", help="if checked, then the connector will add product to salla when create product in Odoo")